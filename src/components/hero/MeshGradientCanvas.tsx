...
vec3 surface(vec2 uv) {
  ...
  if (u_mobile > 0.5) {
    ...
    if (u_layer < 0.5) {
      // Layer 0: Ambient under-veil
      left = bezier(vec2(0.12, -0.25), vec2(0.20 + 0.045 * sway, 0.25), vec2(0.38 + 0.05 * curl, 0.48), vec2(0.78, 1.05), v);
      right = bezier(vec2(0.68, -0.25), vec2(0.78, 0.20), vec2(0.84, 0.65), vec2(1.05, 1.20), v);
    } else if (u_layer < 1.5) {
      // Layer 1: Undercut shadow cavity (narrow crease creating deep 3D depth)
      left = bezier(vec2(0.62, -0.25), vec2(0.64 + 0.05 * sway, 0.18), vec2(0.78, 0.64), vec2(0.80, 1.15), v);
      right = bezier(vec2(0.72, -0.25), vec2(0.76 + 0.04 * curl, 0.23), vec2(0.85, 0.64), vec2(0.96, 1.15), v);
    } else if (u_layer < 2.5) {
      // Layer 2: Primary heroic Trionyx orange ribbon body
      left = bezier(vec2(0.28, -0.25), vec2(0.32 + 0.04 * sway, 0.14), vec2(0.48 + 0.055 * curl, 0.44), vec2(0.75, 0.95), v);
      right = bezier(vec2(0.62, -0.25), vec2(0.72 + 0.025 * curl, 0.20), vec2(0.78 + 0.025 * sway, 0.58), vec2(0.92, 1.15), v);
    } else {
      // Layer 3: Front illuminated golden amber fold rim (crisp gleaming edge)
      left = bezier(vec2(0.54, -0.25), vec2(0.70 + 0.025 * curl, 0.20), vec2(0.78 + 0.025 * sway, 0.60), vec2(0.74, 1.15), v);
      right = bezier(vec2(0.64, -0.25), vec2(0.78 + 0.028 * curl, 0.22), vec2(0.86 + 0.02 * sway, 0.62), vec2(0.82, 1.15), v);
    }
  } else {
...
void main() {
  ...
  if (u_mobile > 0.5) {
    // Graceful bottom dissolve so the cloth flows into the canvas without a hard horizontal cut
    float bottomFade = smoothstep(1.0, 0.70, v_surface.y);
    alpha *= bottomFade;
    alpha *= 0.96;
  } else {
    alpha *= 0.98;
